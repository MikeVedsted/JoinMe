...
const createEvent = async (event: Event) => {
  const newEvent = await (
        await pool.query(
          ...
          [event.title, event.category, event.date, event.time, event.description, event.max_participants, event.address, event.expires_at, event.image]
        )
  )
...
const findAllEvents = async() => {
  const data = await (await pool.query('SELECT * FROM event')).rows
  return data
}
